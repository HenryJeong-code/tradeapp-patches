# PATCH-ID: 2026-03-12-TA-PATCH-CONTROL-PANEL-JS-FIX-02
...
def _record_patch(base_path: str):
    ...
    with open(history_path, "w", encoding="utf-8") as f:
        ...
        f.write("Action: replace setView() block with stable control-panel routing version including patches view\n")
...
def apply_patch(base_path: str):
    ...
    pattern = re.compile(
        r'function setView\(view,\s*symbol\s*=\s*null\)\s*\{.*?\n\s*// -------------------------\n\s*// Coin tabs \(Upbit-ish\)',
        re.S
    )
    ...
    const targetSelector = map[view] || "#view-dashboard";
    ...
    // patches
    if (view === "patches") {
      try { bindPatchViewOnce && bindPatchViewOnce(); } catch (_e) {}
      try { loadPatchPanel && loadPatchPanel(); } catch (_e) {}
    }

    // dossier
    if (view === "dossier") {
      try { window.ViewDossier && window.ViewDossier.bind && window.ViewDossier.bind(); } catch (_e) {}
      try { window.ViewDossier && window.ViewDossier.refresh && window.ViewDossier.refresh(); } catch (_e) {}
    }
